	...
	for face_id, tracker in trackers.items(): # iterate over current trackers
		...
		if sampleNum > 100:
			...
			if (vid == ss):
				...
				conn = mysql.connector.connect(user='root', password='Sudesha79!', host='localhost', database='1drowsydb')
				cursor = conn.cursor()
				cursor.execute("insert into checktb (UserName) values(%s)", (vid,))
				conn.commit()
				conn.close()
				...
				# storing the subject
				msg['Subject'] = "Drowsy Driver DetectionPy"
